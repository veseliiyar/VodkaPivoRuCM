...
public sealed partial class ChargerSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<InsideChargerComponent, ChargeChangedEvent>(OnChargeChanged);
    ...
    public override void Update(float frameTime)
    {
        ...
        var query = EntityQueryEnumerator<ChargerComponent, AppearanceComponent>();
        while (query.MoveNext(out var uid, out var charger, out var appearance))
        {
            var ent = new Entity<ChargerComponent>(uid, charger);
            if (GetStatus(ent) != CellChargerStatus.Charging ||
                !_container.TryGetContainer(uid, charger.SlotId, out var container))
            {
                continue;
            }

            UpdateChargeLevelVisual(ent, container, appearance);
        }
    }
    ...
    private void OnChargeChanged(Entity<InsideChargerComponent> ent, ref ChargeChangedEvent args)
    {
        var chargerUid = Transform(ent).ParentUid;
        if (!TryComp<ChargerComponent>(chargerUid, out var chargerComp) ||
            !_container.TryGetContainer(chargerUid, chargerComp.SlotId, out var container))
        {
            return;
        }

        TryComp<AppearanceComponent>(chargerUid, out var appearance);
        UpdateChargeLevelVisual((chargerUid, chargerComp), container, appearance);
    }
    ...
    /// <summary>
    /// Updates the RMC multi-frame charge indicator independently of the charger's powered status light.
    /// </summary>
    private void UpdateChargeLevelVisual(
        Entity<ChargerComponent> ent,
        ...
        AppearanceComponent? appearance)
    {
        ...
        if (container.ContainedEntities.Count != 0 &&
            _powerCell.TryGetBatteryFromEntityOrSlot(container.ContainedEntities[0], out var battery))
        {
            var charge = _battery.GetChargeLevel(battery.Value.AsNullable());
            var steps = Math.Clamp(ent.Comp.ChargeLevelSteps, 3, byte.MaxValue + 1);
            ...
            else if (charge > 0f)
            {
                ...
                level = (byte) Math.Clamp(
                    (int) Math.Ceiling(charge * intermediateCount),
                    1,
                    intermediateCount);
            }
        }

        _appearance.SetData(ent.Owner, CellVisual.ChargeLevel, level, appearance);
    }
