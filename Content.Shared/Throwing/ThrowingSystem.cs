...
public sealed partial class ThrowingSystem : EntitySystem
{
        ...
        bool rotate = true)
    {
        ...
        {
            Logger.GetSawmill("cmu").Warning($"[TryThrow] Throw invalid direction {direction} entity: {ToPrettyString(uid)}");
            return false;
        }

        if (!float.IsFinite(baseThrowSpeed))
        {
            Logger.GetSawmill("cmu").Warning($"[TryThrow] Throw invalid speed {baseThrowSpeed} entity: {ToPrettyString(uid)}");
            return false;
        }

        if (friction.HasValue && !float.IsFinite(friction.Value))
        {
            Logger.GetSawmill("cmu").Warning($"[TryThrow] Throw invalid friction {friction.Value} entity: {ToPrettyString(uid)} using default");
            friction = null;
        }
