...
/// entity has a <see cref="LightingNightVisionComponent"/> equipped.
/// </summary>
public abstract partial class SharedNightVisionSystem : EntitySystem
{
    ...
    [SubscribeLocalEvent]
    private void OnStartup(Entity<LightingNightVisionComponent> ent, ref MapInitEvent args)
    {
    ...
    [SubscribeLocalEvent]
    private void OnRemove(Entity<LightingNightVisionComponent> ent, ref ComponentShutdown args)
    {
        if (ent.Comp.RelayOverlay)
            return;
    ...
    [SubscribeLocalEvent]
    private void OnCompEquip(Entity<LightingNightVisionComponent> ent, ref GotEquippedEvent args)
    {
        if (!ent.Comp.RelayOverlay)
            return;
    ...
    [SubscribeLocalEvent]
    private void OnCompUnequip(Entity<LightingNightVisionComponent> ent, ref GotUnequippedEvent args)
    {
        if (!ent.Comp.RelayOverlay)
            return;
    ...
    [SubscribeLocalEvent]
    protected virtual void OnRefreshEquipmentHud(Entity<LightingNightVisionComponent> ent, ref InventoryRelayedEvent<RefreshNightVisionEvent> args)
    {
        OnRefreshComponentHud(ent, ref args.Args);
    }

    [SubscribeLocalEvent]
    protected virtual void OnRefreshComponentHud(Entity<LightingNightVisionComponent> ent, ref RefreshNightVisionEvent args)
    {
        if (!ent.Comp.Enabled)
            return;
    ...
    private void OnToggleNightVisionEvent(ToggleNightVisionEvent args)
    {
        ...
        if (!TryComp<LightingNightVisionComponent>(ent, out var nightVisionComp))
            return;
    ...
    public void SetEnabled(Entity<LightingNightVisionComponent?> ent, bool enabled, EntityUid? viewer = null)
    {
...
public record struct RefreshNightVisionEvent() : IInventoryRelayEvent
{
    ...
    public List<Entity<LightingNightVisionComponent>> Entities = new();
}
