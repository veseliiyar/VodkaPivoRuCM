...
public sealed partial class TriggerSystem
{
    ...
    private void InitializeCondition()
    {
        ...
        SubscribeLocalEvent<WornSlotTriggerConditionComponent, AttemptTriggerEvent>(OnWornSlotTriggerAttempt);

        SubscribeLocalEvent<ToggleTriggerConditionComponent, GetVerbsEvent<AlternativeVerb>>(OnToggleGetAltVerbs);
    }

    private void OnWornSlotTriggerAttempt(Entity<WornSlotTriggerConditionComponent> ent, ref AttemptTriggerEvent args)
    {
        if (args.Key != null && !ent.Comp.Keys.Contains(args.Key))
            return;

        args.Cancelled |= !_inventory.InSlotWithFlags((ent.Owner, null, null), ent.Comp.RequiredSlots);
    }
