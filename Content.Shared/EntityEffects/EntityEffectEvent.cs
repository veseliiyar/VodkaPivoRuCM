...
/// <param name="ReagentContext">Optional chemistry-specific application context.</param>
[ByRefEvent, Access(typeof(SharedEntityEffectsSystem))]
public readonly record struct EntityEffectEvent<T>(
    T Effect,
    float Scale,
    EntityUid? User,
    ReagentEffectContext? ReagentContext) where T : EntityEffectBase<T>
{
    /// <summary>
    ...
    /// <summary>
    /// Chemistry-specific context when this effect came from a reagent.
    /// </summary>
    public readonly ReagentEffectContext? ReagentContext = ReagentContext;
}
