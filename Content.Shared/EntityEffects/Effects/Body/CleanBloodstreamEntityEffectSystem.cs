...
public sealed partial class CleanBloodstreamEntityEffectSystem : EntityEffectSystem<BloodstreamComponent, CleanBloodstream>
{
    ...
    {
        var excluded = args.Effect.Excluded;
        if (args.ReagentContext is { } context)
        {
            if (context.Source == null)
                return;

            excluded = context.Reagent.ID;
        }

        var amount = args.Effect.CleanseRate * args.Scale;
        _bloodstream.FlushChemicals((entity, entity), amount, excluded);
    }
}
