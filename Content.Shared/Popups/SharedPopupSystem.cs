...
public abstract partial class SharedPopupSystem : EntitySystem
{
    ...
    /// <summary>
    /// Returns whether a popup associated with <paramref name="recipient"/> should also be broadcast to other players.
    /// The recipient's own popup is never gated by this method.
    /// </summary>
    protected virtual bool ShouldBroadcastToOthers(EntityUid recipient)
    {
        return true;
    }
        ...
        PopupType type = PopupType.Small)
    {
        ...
        {
            if (ShouldBroadcastToOthers(recipient.Value))
