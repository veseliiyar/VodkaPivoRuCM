...
public sealed partial class PlantMutationSystem : EntitySystem
{
    ...
    public void CheckRandomMutations(Entity<PlantComponent?> ent, float severity)
    {
        ...
        {
            var ev = new BeforeRandomPlantMutationEvent(ent.Owner, mutation);
    ...
    public void SpeciesChange(Entity<PlantDataComponent?> oldPlant, EntProtoId newPlantProto)
    {
        ...
        var changed = new PlantSpeciesChangedEvent(oldPlant.Owner, newPlantUid);
...
/// <summary>
/// Raised for each random mutation candidate before its probability roll and application.
/// </summary>
[ByRefEvent]
public record struct BeforeRandomPlantMutationEvent(
    EntityUid Plant,
    RandomPlantMutation Mutation,
    bool Cancelled = false);

/// <summary>
/// Raised after a species mutation has copied the plant lifecycle into its replacement entity.
/// </summary>
[ByRefEvent]
public readonly record struct PlantSpeciesChangedEvent(EntityUid OldPlant, EntityUid NewPlant);
