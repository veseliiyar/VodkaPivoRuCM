...
public sealed partial class AnchorableSystem : EntitySystem
{
    ...
    public bool CanAnchorAt(Entity<PhysicsComponent?> entity, EntityCoordinates coordinates, EntityUid? user = null)
    {
        ...
        if (TileFree(coordinates, entity.Comp, entity.Owner)) // RMC14: exclude the entity being anchored.
            return true;
    ...
    public bool TileFree(Entity<MapGridComponent> grid, Vector2i gridIndices, int collisionLayer = 0, int collisionMask = 0, EntityUid? anchoringEntity = null) //RMC14
    {
        ...
        {
            // RMC14: an already-anchored entity must not collide with itself when revalidating its tile.
            if (ent == anchoringEntity)
                continue;
