...
public abstract partial class SharedStackSystem
{
    ...
    )
    {
        ...
        // RMC: Stacks covered in acid cannot be merged.
        ...
        // RMC: This event must run after both count changes so folding-barricade
        // damage state can follow the transferred stack items.
        var merged = new StackMergedEvent(donor.Owner, recipient.Owner, transferred);
        RaiseLocalEvent(recipient.Owner, ref merged);
        return true;
    }

    /// <summary>
    /// Compatibility wrapper for callers using the previous stack API.
    /// </summary>
    [Obsolete("Use TryMergeStacks instead")]
    public bool TryAdd(
        EntityUid insertEnt,
        EntityUid targetEnt,
    ...
    {
        return TryMergeStacks((insertEnt, insertStack), (targetEnt, targetStack), out _);
    }
    ...
    public bool TryAdd(
        ...
        StackComponent? targetStack = null)
    {
        return TryMergeStacks((insertEnt, insertStack), (targetEnt, targetStack), out _, count);
    }
