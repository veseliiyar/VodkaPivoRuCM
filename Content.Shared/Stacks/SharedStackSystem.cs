...
public abstract partial class SharedStackSystem : EntitySystem
{
        ...
        if (TryGetAcidedStack(ent.Owner, args.Used, out var acidUid))
        {
            if (_gameTiming.IsFirstTimePredicted)
            {
                ...
                if (!acidPopupPos.IsValid(EntityManager))
                    acidPopupPos = Transform(args.User).Coordinates;
    ...
    private void OnStackAlternativeInteract(Entity<StackComponent> ent, ref GetVerbsEvent<AlternativeVerb> args)
    {
        ...
        // RMC: Acided stacks cannot be split.
        if (IsAcidicStack(ent.Owner))
            return;
    ...
    /// <summary>
    /// Checks whether a stack is currently being corroded by xeno acid.
    /// </summary>
