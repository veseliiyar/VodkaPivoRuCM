...
public sealed partial class RMCReflectSystem : EntitySystem
{
    ...
    private void OnProjectileReflectAttempt(Entity<RMCReflectiveComponent> ent, ref ProjectileReflectAttemptEvent args)
    {
        ...
        if (TryComp(projectile, out RMCReflectedProjectileComponent? reflected) &&
            reflected.ReflectedBy.Contains(reflectorId))
        {
        ...
        if (!TryReflectProjectile(projectile, ent, ent.Comp.Angle, ent.Comp.Chance))
            return;

        if (TryComp(projectile, out ProjectileIFFComponent? iff) && iff.Enabled)
        {
            args.Component.Damage *= 0;
            return;
        }

        reflected = EnsureComp<RMCReflectedProjectileComponent>(projectile);
