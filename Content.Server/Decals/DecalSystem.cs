...
public sealed partial class DecalSystem : SharedDecalSystem
{
    ...
    /// <summary>
    /// Returns whether the grid still owns the exact chunk-local decal index.
    /// </summary>
    public bool ContainsDecal(EntityUid gridUid, DecalIndex decalId)
        => TryGetDecalChunk(gridUid, decalId, out _);

    /// <summary>
    /// Returns a snapshot of every decal currently owned by a grid.
    /// </summary>
    public List<(DecalIndex Index, Decal Decal)> GetAllDecals(EntityUid gridUid)
    {
        var result = new List<(DecalIndex, Decal)>();
        foreach (var chunk in ChunkEntities.GetChunks(gridUid))
        {
            if (!DecalChunkQuery.TryComp(chunk.Owner, out var decals))
                continue;

            foreach (var (id, decal) in decals.Decals)
                result.Add((new DecalIndex(chunk.Comp.Chunk, id), decal));
        }
    ...
    /// <summary>
    /// Removes matching decals from a grid through the canonical chunk-entity removal path.
    /// </summary>
    public (int Removed, int Skipped) RemoveDecals(
        EntityUid gridId,
        IReadOnlySet<string>? ids = null,
        bool cleanableOnly = true)
    {
        var toRemove = new List<DecalIndex>();
        var skipped = 0;

        foreach (var chunk in ChunkEntities.GetChunks(gridId))
        {
            if (!DecalChunkQuery.TryComp(chunk.Owner, out var decals))
                continue;
            ...
            foreach (var (id, decal) in decals.Decals)
            {
                if (ids != null && !ids.Contains(decal.Id))
                    continue;
                ...
                toRemove.Add(new DecalIndex(chunk.Comp.Chunk, id));
            }
        }

        var removed = 0;
        foreach (var decal in toRemove)
        {
            if (RemoveDecal(gridId, decal))
                removed++;
        }
