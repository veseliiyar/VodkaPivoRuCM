...
public sealed partial class BiomorphLeapSystem : EntitySystem
{
    ...
    {
        SubscribeLocalEvent<BiomorphLeapComponent, BiomorphLeapActionEvent>(OnLeapAction);
        SubscribeLocalEvent<BiomorphLeapingComponent, StartCollideEvent>(OnLeapingCollide);
        SubscribeLocalEvent<BiomorphLeapingComponent, ComponentRemove>(OnLeapingRemove);
        SubscribeLocalEvent<BiomorphLeapingComponent, PhysicsSleepEvent>(OnLeapingPhysicsSleep);
    }
    ...
    public override void Update(float frameTime)
    {
        ...
        EntityQueryEnumerator<BiomorphLeapingComponent> query = EntityQueryEnumerator<BiomorphLeapingComponent>();
        while (query.MoveNext(out EntityUid uid, out BiomorphLeapingComponent? leaping))
        {
    ...
    private void OnLeapAction(Entity<BiomorphLeapComponent> ent, ref BiomorphLeapActionEvent args)
    {
        if (args.Handled)
            return;
        ...
        if (HasComp<BiomorphLeapingComponent>(ent))
            return;
        ...
        var leaping = EnsureComp<BiomorphLeapingComponent>(ent);
    ...
    private void OnLeapingCollide(Entity<BiomorphLeapingComponent> ent, ref StartCollideEvent args)
    {
        ApplyHit(ent, args.OtherEntity);
    }

    private bool ApplyHit(Entity<BiomorphLeapingComponent> ent, EntityUid target)
    {
    ...
    private void OnLeapingPhysicsSleep(Entity<BiomorphLeapingComponent> ent, ref PhysicsSleepEvent args)
    {
        StopLeap(ent);
    }

    private void OnLeapingRemove(Entity<BiomorphLeapingComponent> ent, ref ComponentRemove args)
    {
        if (TryComp(ent, out PhysicsComponent? physics))
        {
            _physics.SetLinearVelocity(ent, Vector2.Zero, body: physics);
    ...
    private void StopLeap(Entity<BiomorphLeapingComponent> ent)
    {
        RemCompDeferred<BiomorphLeapingComponent>(ent);
    }
}
