...
/// </summary>
public sealed partial class KillAllBiomorphsRuleSystem : GameRuleSystem<KillAllBiomorphsRuleComponent>
{
    ...
    {
        if (_gameTicker.IsGameRuleActive<KillAllBiomorphsRuleComponent>())
    ...
    {
        EntityQueryEnumerator<ActiveGameRuleComponent, KillAllBiomorphsRuleComponent, GameRuleComponent>
            queryRule = QueryActiveRules();
        if (!ThreatRuleHelper.TryGetActiveRule(ref queryRule, out KillAllBiomorphsRuleComponent ruleComp, out _))
            return;
        ...
        {
            bool isAbom = _entMan.HasComponent<BiomorphComponent>(uid)
                || _entMan.HasComponent<BiomorphMimicTransformedComponent>(uid);
            if (!isAbom)
                continue;
        ...
        _roundStats.RecordThreatDefeatedRule("KillAllBiomorphsRule");
