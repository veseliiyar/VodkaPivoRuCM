...
public abstract partial class GameTest
{
    ...
    {
        // Client-only entities may be attached to replicated parents, so clear them before
        // authoritative deletion states begin arriving from the server.
        ...
        // Replicate each hierarchy leaf-first across separate game states. Deleting a root and
        // its children together makes the client detach children while enumerating that same set.
        foreach (var junk in _serverEntitiesToClean)
        {
            await Pair.DeleteEntityTreeLeafFirst(junk);
        }
    }
