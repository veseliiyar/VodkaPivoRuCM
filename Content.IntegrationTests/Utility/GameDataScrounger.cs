...
public static partial class GameDataScrounger
{
    ...
    public static string[] EntitiesWithComponent(string componentId)
    {
        ...
        lock (DataLock)
        {
            ...
            {
                return index.TryGetValue(componentId, out var entities)
                    ? entities.ToArray()
                    : Array.Empty<string>();
            }
            else
            {
                ...
                return _entitiesWithComponentIndex.TryGetValue(componentId, out var entities)
                    ? entities.ToArray()
                    : Array.Empty<string>();
            }
        }
    ...
    private static IEnumerable<(string type, string id)> IndexPrototypesIn(string file, bool ignored = false)
    {
        ...
        foreach (var document in stream)
        {
            ...
            foreach (var entry in node.Children)
            {
                ...
                string[] ids = id switch
                {
                    YamlScalarNode scalar => [scalar.Value!],
                    YamlMappingNode variants when variants.TryGetNode("values", out YamlSequenceNode? values) =>
                        values.Children.Select(value => value.AsString()).ToArray(),
                    _ => [],
                };
                ...
                if (!@abstract)
                {
                    foreach (var prototypeId in ids)
                    {
                        yield return (((YamlScalarNode)type).Value!, prototypeId);
                    }
                }
                ...
                foreach (var prototypeId in ids)
                {
                    ...
                    _entitiesMetaIndex![prototypeId] = entity;
                }
            }
        }
    }
    ...
    private static void VisitEntity(EntityMetadata entity, in HashSet<string> visitedEntities)
    {
        ...
        {
            // Removed prototype IDs can remain as inherited parents while migrations redirect
            // serialized map references to their successors. They do not contribute raw metadata.
