...
public sealed partial class LoadoutGroupContainer : BoxContainer
{
    ...
    public void RefreshLoadouts(HumanoidCharacterProfile profile, RoleLoadout loadout, ICommonSession session, IDependencyCollection collection)
    {
        ...
        foreach (var kvp in groups)
        {
            ...
            if (protos.Count > 1)
            {
                ...
                */
                var firstElement = uiElements.FirstOrDefault(e => e.IsSelected) ?? uiElements[0];
    ...
    private LoadoutContainer CreateLoadoutUI(LoadoutPrototype proto, HumanoidCharacterProfile profile, RoleLoadout loadout, ICommonSession session, IDependencyCollection collection, LoadoutSystem loadoutSystem)
    {
        ...
        var selectedCount = selected.Count(e => e.Prototype == proto.ID);
        var pressed = selectedCount > 0;
        var maxSelections = Math.Max(1, proto.MaxSelections);
        var repeatable = maxSelections > 1;

        var enabled = loadout.IsValid(profile, session, proto.ID, collection, out var reason);

        if (repeatable && selectedCount >= maxSelections)
        {
            reason = FormattedMessage.FromUnformatted(Loc.GetString("loadouts-selection-limit-reached"));
            enabled = false;
        }

        if (proto.Cost != null && loadout.Points != null)
        {
            if (!repeatable && loadout.Points < proto.Cost && pressed)
            {
                reason = null;
                enabled = true;
            }
        ...
        var cont = new LoadoutContainer(proto, !enabled, reason, repeatable)
        {
            IsSelected = pressed,
        };

        var name = loadoutSystem.GetName(proto);
        if (proto.Effects.OfType<SkillLoadoutEffect>().FirstOrDefault() is { } skillEffect)
            name = Loc.GetString("loadouts-skill-upgrade-name", ("name", name), ("amount", skillEffect.Amount));

        cont.Text = (repeatable, proto.Cost) switch
        {
            (true, { } cost) => Loc.GetString("loadouts-repeatable-name-cost",
                ("name", name),
                ("count", selectedCount),
                ("max", maxSelections),
                ("cost", cost)),
            (true, _) => Loc.GetString("loadouts-repeatable-name",
                ("name", name),
                ("count", selectedCount),
                ("max", maxSelections)),
            _ => name,
        ...
        cont.Select.Pressed = !repeatable && pressed;
        cont.Remove.Visible = repeatable && pressed;

        cont.Select.OnPressed += args =>
        {
            if (repeatable || args.Button.Pressed)
                OnLoadoutPressed?.Invoke(proto.ID);
            else
                OnLoadoutUnpressed?.Invoke(proto.ID);
        };

        cont.Remove.OnPressed += _ => OnLoadoutUnpressed?.Invoke(proto.ID);
