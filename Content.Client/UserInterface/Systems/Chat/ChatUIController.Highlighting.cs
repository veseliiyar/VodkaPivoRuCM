...
public sealed partial class ChatUIController : IOnSystemChanged<CharacterInfoSystem>
{
    ...
    public void ReloadHighlights()
    {
        ...
        for (var i = 0; i < splittedHighlights.Length; i++)
        {
            ...
            keyword = StartAtSign.Replace(keyword,
                $@"(?:{AfterSpeakerHighlightContext}|(?<=(?<=(L?OOC|DEAD|ADMIN):.*:.*)|(?<=,.*{_chatSpeechDoubleQuoteBegin}.*)|(?<=\n.*)))");
    ...
    private void OnCharacterUpdated(CharacterData data)
    {
        ...
        var (_, _, _, jobId, jobTitle, entityName, _) = data;
        ...
        {
            var dynamicJobKey = jobTitle.Replace(' ', '-').ToLowerInvariant();
            _loc.TryGetString($"highlights-{dynamicJobKey}", out jobMatches);
        }

        if (jobMatches == null && jobId is { } id)
        {
            var jobKey = id.Id.Replace(' ', '-').ToLowerInvariant();
            _loc.TryGetString($"highlights-{jobKey}", out jobMatches);
        }
