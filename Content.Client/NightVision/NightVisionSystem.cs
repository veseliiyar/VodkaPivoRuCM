...
public sealed partial class NightVisionSystem : SharedNightVisionSystem
{
    ...
    [SubscribeLocalEvent]
    private void OnHandleState(Entity<LightingNightVisionComponent> ent, ref AfterAutoHandleStateEvent args)
    {
        RefreshOverlay(ent);
    }
    ...
    private void Update(EntityUid entity, List<Entity<LightingNightVisionComponent>> entities)
    {
        if (entity != _player.LocalSession?.AttachedEntity)
            return;

        // Find the component with the lowest noise.
        LightingNightVisionComponent? nvision = null;
