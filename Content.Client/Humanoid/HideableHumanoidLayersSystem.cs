...
public sealed partial class HideableHumanoidLayersSystem : SharedHideableHumanoidLayersSystem
{
    ...
    public override void SetLayerOcclusion(
        ...
        SlotFlags source)
    {
        base.SetLayerOcclusion(ent, layer, hidden, source);
    ...
    /// <inheritdoc />
    public override void SetPermanentLayerOcclusion(
        Entity<HideableHumanoidLayersComponent?> ent,
        HumanoidVisualLayers layer,
        bool hidden)
    {
        base.SetPermanentLayerOcclusion(ent, layer, hidden);
    ...
    {
        var occludedLayers = new HashSet<HumanoidVisualLayers>(ent.Comp.HiddenLayers.Keys);
        occludedLayers.UnionWith(ent.Comp.PermanentlyHiddenLayers);

        // Reset old hidden layers if they aren't in our new set.
        foreach (var item in ent.Comp.LastHiddenLayers)
        {
            if (occludedLayers.Contains(item))
                continue;
        ...
        var actualHiddenLayers = new HashSet<HumanoidVisualLayers>(occludedLayers.Count);

        // Handle hiding our new layers - handlers must set ShouldHide true to hide the layer.
        foreach (var item in occludedLayers)
        {
