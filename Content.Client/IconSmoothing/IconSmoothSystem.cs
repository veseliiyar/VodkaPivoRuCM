    ...
    public sealed partial class IconSmoothSystem : EntitySystem
    {
                ...
                || !smooth.Running)
            {
                    ...
                    TryComp(uid, out xform))
                {
                    ...
                    if (TryComp(xform.GridUid, out MapGridComponent? grid))
                    {
                        ...
                        if (MatchingEntity(uid, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.North))))
                            directions |= DirectionFlag.North;
                        if (MatchingEntity(uid, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.South))))
                            directions |= DirectionFlag.South;
                        if (MatchingEntity(uid, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.East))))
                            directions |= DirectionFlag.East;
                        if (MatchingEntity(uid, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.West))))
            ...
            Entity<SpriteComponent> sprite, TransformComponent xform)
        {
            ...
            for (var i = 0; i < neighbors.Length; i++)
            {
                ...
                matching = matching && MatchingEntity(sprite.Owner, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos + neighbor));
            }
        ...
        private void CalculateNewSpriteCardinal(Entity<MapGridComponent>? gridEntity, IconSmoothComponent smooth, Entity<SpriteComponent> sprite, TransformComponent xform)
        {
            ...
            var pos = _mapSystem.TileIndicesFor(gridUid, grid, xform.Coordinates);
            if (MatchingEntity(sprite.Owner, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.North))))
                dirs |= CardinalConnectDirs.North;
            if (MatchingEntity(sprite.Owner, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.South))))
                dirs |= CardinalConnectDirs.South;
            if (MatchingEntity(sprite.Owner, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.East))))
                dirs |= CardinalConnectDirs.East;
            if (MatchingEntity(sprite.Owner, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.West))))
        ...
        private bool MatchingEntity(EntityUid uid, IconSmoothComponent smooth, AnchoredEntitiesEnumerator candidates)
        {
            var cmSmooth = _cmIconSmoothQuery.TryComp(uid, out var cmComponent) && cmComponent.Smooth;
            ...
            while (candidates.MoveNext(out var entity))
            {
                if (_iconSmoothQuery.TryGetComponent(entity, out var other) &&
                    ...
                    (other.SmoothKey == smooth.SmoothKey ||
                     ...
                     cmSmooth && _cmIconSmoothQuery.HasComp(entity)))
                {
                    return true;
                }
            }
        ...
        private void CalculateNewSpriteCorners(Entity<MapGridComponent>? gridEntity, IconSmoothComponent smooth, Entity<SpriteComponent> spriteEnt, TransformComponent xform)
        {
            var (cornerNE, cornerNW, cornerSW, cornerSE) = gridEntity == null
                ...
                : CalculateCornerFill(spriteEnt.Owner, gridEntity.Value, smooth, xform);
        ...
        private (CornerFill ne, CornerFill nw, CornerFill sw, CornerFill se) CalculateCornerFill(EntityUid uid, Entity<MapGridComponent> gridEntity, IconSmoothComponent smooth, TransformComponent xform)
        {
            var gridUid = gridEntity.Owner;
            var grid = gridEntity.Comp;

            var pos = _mapSystem.TileIndicesFor(gridUid, grid, xform.Coordinates);
            var n = MatchingEntity(uid, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.North)));
            var ne = MatchingEntity(uid, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.NorthEast)));
            var e = MatchingEntity(uid, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.East)));
            var se = MatchingEntity(uid, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.SouthEast)));
            var s = MatchingEntity(uid, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.South)));
            var sw = MatchingEntity(uid, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.SouthWest)));
            var w = MatchingEntity(uid, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.West)));
            var nw = MatchingEntity(uid, smooth, _mapSystem.GetAnchoredEntities(gridUid, grid, pos.Offset(Direction.NorthWest)));
